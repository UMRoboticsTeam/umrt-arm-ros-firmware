...
WheelAdapter::WheelAdapter(const std::string& can_interface, const std::vector<uint16_t>& motor_ids, const std::chrono::duration<int64_t, std::milli>& query_period) : StepperAdapter(motor_ids.size()) {
    ...
    //     that WheelController can use for its packet address lookups
    ...
    controller = std::make_unique<WheelController>(can_interface, std::move(motor_ids_for_controller), NORM_FACTOR);
...
void WheelAdapter::queryController() {
    for (auto i = 0u; i < commands.size(); ++i) {
        this->controller->getPosition(this->motor_ids->left.at(i));
    }
}

void WheelAdapter::queryPoll(const std::chrono::milliseconds& period) {
